...
def eda_produccion():
    ...
    if vista == "Producción":
        # escala logaritmica
        st.sidebar.write('Escala de Producción anual')
        ...
        with pcol1:
            ...
            if s_log == False: # implicito que puede cambiar de escala, preguntar para otras formas
                title_escala = f"Total anual producido por comunidad, en {year_query} (escala real)"
            else:
        ...
        # sidebar Eficiencia ------------------------------------------
        # escala logaritmica
        st.sidebar.write('Escala de Producción anual/superficie')
        s_log = st.sidebar.checkbox("Escala Logarítmica")
        ...
        if comun_prov == 'Provincia':
            ...
            # escala solo para provincias desagrupadas
            s_log_p = st.sidebar.checkbox("Escala Logarítmica (provincias)")
        else:
            # activa overlay
            st.sidebar.write('Vista agrupada de provincias')
            bar_mode = st.sidebar.checkbox("Overlay")
            if bar_mode == True:
                bar_overlay= 'overlay'
            else:
                bar_overlay = 'group'
        # cierre sidebar Eficiencia ------------------------------------

        # graficos
        gefcol1, gefcol2 = st.columns([1,1])
        with gefcol1:
            # graficos generales de vista de eficiencia anuales
            # grafico general de produccion de cada comunidad
        ...
        with gefcol2:
            if s_log == False: # implicito que puede cambiar de escala
                title_escala = "Producción total en relación a la superficie de plantación (escala real)"
            else:
                title_escala = "Producción total en relación a la superficie de plantación, en escala logarítmica"
            # grafico general de produccion / superficie (todas las comunidades)
        ...
        if comun_prov == 'Comunidad':
            ...
            fig_prod_eficiency_com = px.bar(df_prod[df_prod["Comunidad"]==autonomic_query],
                                        x= ("Año"),
                                        y= ("Eficiencia"),
                                        hover_name= "Provincias",
            ...
            fig_precip_com = px.bar(data_frame= df_prod[df_prod["Comunidad"]== autonomic_query],
                                x= "Año",
                                y= "precip",
                                color= "precip",
                                hover_name = 'Provincias',
            ...
            fig_prod_temp_com = px.scatter(df_prod[df_prod['Comunidad']==autonomic_query],
                                       x           = "Producción Total",
                                       y           = "temp",
                                       ...
                                       template = 'plotly_dark')
            fig_prod_temp_com.add_hline(y= 15,
                                        #line_dash= "dot",
                                        annotation_text=('Temparatura mínima ideal (general)'),
                                        annotation_position= "top left")
        ...
        # provincias
            ...
            fig_prod_eficiency_prov = px.bar(df_prod[df_prod["Provincias"]==province_query],
                                        x= ("Año"),
                                        y= ("Eficiencia"),
                                        hover_name= "Provincias",
                                        hover_data = ['Producción Total'],
            ...
            fig_precip_prov = px.bar(df_prod[df_prod["Provincias"]== province_query],
                                    x= "Año",
                                    y= "precip",
                                    log_y = s_log_p,
                                    color= "precip",
            ...
            st.write('Para poder cubrir requerimientos del recurso hidrico en la mayoria de las provincias, los agricultores recurren a tecnicas de riego, las cuales permiten solventar la escacez de las lluvias según sea el caso, es por ello que las producciones totales no se ven afectadas cuando hay pocas precipitaciones.')
